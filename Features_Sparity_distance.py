#coding:utf-8
import random
import numpy as np
import cv2
import matplotlib.pyplot as plt
img = cv2.imread('test2.png',0)
#delete_freq=5 　　　　　　　　
i=2
akaze=cv2.AKAZE_create(threshold=0.00005) #smaller, more points
kp_akaze = akaze.detect(img,None)        #keypoints of akaze
img_akaze = cv2.drawKeypoints(img,kp_akaze,img,color=(255,0,0))
cv2.imshow('AKAZE',img_akaze)
cv2.waitKey(0)
def dist(x1,x2,y1,y2):
    return ((x1-x2)**2+(y1-y2)**2)**0.5

pts=cv2.KeyPoint_convert(kp_akaze)        #positions of keypoints
while i < len(kp_akaze)-6:
    x1=pts[i][0]
    y1=pts[i][1]

    x2=pts[i-2][0]
    y2=pts[i-2][1]

    x3=pts[i+2][0]
    y3=pts[i+2][1]

    dist1=dist(x1,x2,y1,y2)
    dist2=dist(x1,x3,y3,y3)
    print(dist1)
    print(dist2)

    if dist1+dist2>100:    #smaller,more kp 
        cv2.circle(img,(pts[i][0],pts[i][1]),2,(225,153,18),thickness=2) #draw pts[i][0] circle in the image
    i=i+5
cv2.imshow('AKAZE_2',img)
cv2.waitKey(0)
